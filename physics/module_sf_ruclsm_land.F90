...
! soilprop is called from subroutine SFCTMP which is called from subroutine RUC_land;
...
!-----------------------------------------------------------------
!>\ingroup lsm_ruc_land_group
!> The RUC LSM model is described in Smirnova et al.(1997) 
...
!--- soil/snow properties
   ...
   REAL,     DIMENSION(1:2*(nsl-1))          ::           DTDZS
...
!
         ...
         !-----
         IF ( debug_print ) THEN
           print *,' ZSMAIN, DZMAIN, zsmain_dop --->', zsmain,dzmain,zsmain_dop
           print *,' ZSHALF, DZhalf, zshalf_dop --->', zshalf,dzhalf,zshalf_dop
         ENDIF
         !------------------------------------------------------------
...
!***********************************************************************
         ...
         NROOT= 4 ! rooting depth

         RHONEWSN = 100.
         CN=CFACTR_DATA   ! exponent
         SAT = 5.e-4  ! canopy saturation [m]
...
!---- table TBQ is for the solution of balance equation in VILKA
        CQ=173.15-.05
        R273=1./273.15
        R61=6.1153*0.62198
...
!--- This is temporary until SI is added to mass coordinate ---!!!!!
     ...
     DO J=jts,jte
         DO i=its,ite
            ...
            !--- initializing inside snow temp 
            IF((soilt1(i,j) .LT. 200.) .or. (soilt1(i,j) .GT.350.)) THEN
            ...
            IF((qvg(i,j) .LE. 0.) .or. (qvg(i,j) .GT.0.1)) THEN
              ...
              IF ( debug_print ) THEN
                WRITE ( message , FMT='(A,3F8.3,2I6)' ) &
                  'QVG is initialized in RUCLSM ', qvg(i,j),qv3d(i,1,j),qsg(i,j),i,j
                WRITE ( 0 , message )
              ENDIF
            ENDIF
            ...
            if(SNOW(i,j).gt.0. .and. SNOWH(i,j).gt.0.) then
              RHOSN(i,j) = max(58.8,min(500.,SNOW(i,j)/SNOWH(i,j)) )
            else
              RHOSN(i,j) = 300.
            endif
            ...
            !-- For RUC LSM CHKLOWQ needed for MYJPBL should be 1 
            chklowq(i,j) = 1.
            infiltr(i,j) = 0.
            snoh  (i,j) = 0.
            edir  (i,j) = 0.
     ...
     endif  ! end of initizlization at ktau=1 
     ...
     DO i=its,ite
       ...
       ! LAND POINT OR SEA ICE
         ...
         IF ( debug_print ) THEN
           ...
           print *,' IVGTYP, ISLTYP, ALB ',i,j,ivgtyp(i,j),isltyp(i,j),alb(i,j)
           print *,' MAVAIL, XLAND ', i,j,xland(i,j),mavail(i,j)
           print *,' SOILT,QVG,P8w',soilt(i,j),qvg(i,j),p8w(i,1,j)
           print *, 'LSMRUC, I,J, QFX,HFX from SFCLAY',i,j, qfx(i,j),hfx(i,j)
           print *, 'after SFCLAY I,J,CHS,FLHC ',i,j,chs(i,j),flhc(i,j)
           ...
           print *, 'I,J,DT,RAINBL =',I,J,dt,RAINBL(i,j)
         ENDIF
       ...
       !-- Partition precipitation into liquid and frozen phases
       IF(FRPCPN) THEN
       ...
       ENDIF
        ...
        ! -- save time-step water equivalent of frozen precipitation in PRECIPFR 
        ! -- to be used in module_diagnostics
        precipfr(i,j) = NEWSNMS * DT *1.e3 ! [mm]
...
!> - Call soilvegin() to initialize soil and surface properties
    ...
    ENDIF
     ...
     !-- Canopy saturation dependence on LAI, turned off for now
     !SAT=max(1.e-5,(min(5.e-4,(CMCMAX_DATA * (1.-exp(-0.5*lai(i,j))) * 0.01*VEGFRA(I,J)))))

     !-- define the number of soil levels in the rooting zone
...
!beka
!stochastic perturbation at time step 1
    if (spp_lsm == 1) then
        IF (KTAU == 2) THEN
            do k=1,nzs
               SOILMOIS(i,k,j)=SOILMOIS(i,k,j)*(1+1.5*rstoch(i,k,j))
            enddo
        ENDIF
    endif
!beka
...
!--- input variables
                ...
                snoalb(i,j),albbck(i,j),lai(i,j),tso_dop,        &   !new
                zshalf_dop,myjpbl,isice,                         &
!--- soil fixed fields
                QWRTZ,                                           &
                rhocs,dqm,qmin,ref,                              &
                wilt,psis,bclh,ksat,                             &
...
!--- constants
                ...
                KQWRTZ,KICE,KWT,                                 &
!--- output variables
                rsm,soilm1d,tso1d,smfrkeep,keepfr,               &
                soilt(I,J),soilt1(i,j),tsnav(i,j),dew(I,J),      &
                qvg(I,J),qsg(I,J),qcg(I,J),                      &
...
! evan
       ...
       !-- send RHOSN to GRDFLX for displaying in wrfout*
       !GRDFLX(I,J) = RHOSN(i,j)

       !-- for test put heat budget term in GRDFLX
...
!--- constants
                ...
                KQWRTZ,KICE,KWT,                                 &
!--- output variables
                rsm,soilm1d,ts1d,smfrkeep,keepfr,                &
                soilt,soilt1,tsnav,dew,qvg,qsg,qcg,              &
                SNOH,SNFLX,SNOM,SNOWFALLAC,ACSNOW,               &
...
!--- Local variables
   ...
   REAL    ::  BSN, XSN, fso, fsn                              , &
               RAINF, SNTH, NEWSN, PRCPMS, NEWSNMS             , &
               SNWEPR, T3, UPFLUX, XINET
   REAL    ::  rhocsn, keff, tsob,snprim,d9,d9sn,deltsn,fact
...
!-----------------------------------------------------------------
       ...
       IF ( debug_print ) THEN
         print *,' in SFCTMP i,j,nzs,nddzs,nroot,SNWE,RHOSN,SNOM,SMELT,TS1D', &
                             i,j,nzs,nddzs,nroot,SNWE,RHOSN,SNOM,SMELT,TS1D
       ENDIF
...
!beka
       if (spp_lsm == 1) then
         VEGFRAC = min(VEGFRAC * (1. + 0.33*rstochcol(1)), 1.)
       endif

! evan
       if (spp_lsm == 1) then
         alb = min(ALBTBL(IVGTYP) * (1. + 0.4*rstochcol(1)),1.)
       endif
...
!      endif
       ...
       !-- SNHEI_CRIT_newsn is a threshold for new snow that
       !-- defines if we want to keep maximum snow albedo
       !-- value recommended in literature is 3 mm of SWE.
       !SNHEI_CRIT_newsn=0.003*1.e3/rhosn ! Col DePorte obs
       ...
       if(snhei.gt.0.0081*1.e3/rhosn) then
         !print *,'1- Temp rhosn',i,j,rhosn,tsnav
         ...
         !print *,'2- Temp rhosn',i,j,rhosn,tsnav
   777   continue
       endif

       newsn=newsnms*delt ! [m] of SWE
       !-- ACSNOW - run-total snowfall water [mm]
       !acsnow=acsnow+newsn*1.e3

       !--- Compute density of frozen precipitation
       IF(NEWSN.GT.0.) THEN
         ...
         !-- from here rhonewsn is the density of falling frozen precipitation
         rhonewsn=rhosnfall

         !--- Update average snow density of the snow pack considering
         !--- the the density of fresh snow (eq. 9 in Koren et al.(1999) 
         !--- without snow melt )
         !print *,'1- Newsn rhosn',i,j,rhosn,snwe,newsn,rhonewsn
         xsn=(rhosn*snwe+rhonewsn*newsn)/                         &
             (snwe+newsn)
         rhosn=MIN(MAX(58.8,XSN),500.)
         !print *,'2- Newsn rhosn',i,j,rhosn,snwe,newsn,rhonewsn
       ENDIF ! end NEWSN > 0.

       !--- CALCULATION OF CANOPY WATER (Smirnova et al., 1997, EQ.16)
       intwratio=0.

       !--- compute intercepted precipitation - Eq. 1 Lawrence et al.,
       !--- J. of Hydrometeorology, 2006, CLM.
       if(vegfrac > 0.01) then
         interw=0.25*DELT*PRCPMS*(1.-exp(-0.5*lai))*vegfrac ! liquid
         intersn=0.25*NEWSN*(1.-exp(-0.5*lai))*vegfrac ! frozen
         ...
         IF(CST.GT.SAT) THEN
           ...
           dripsn = drip - dripliq
           infwater=infwater+dripliq/delt
         ENDIF
       ...
       !-- Update SWE on the ground with new snow minus intercepted by canopy
       !-- and plus drip 
       if(newsn > 0.) then
           snwe=max(0.,snwe+newsn-intersn+dripsn)
           snhei=snwe*rhowater/rhosn
       endif ! newsn

       !-- convert new snow from SWE to snow depth
       ...
       !-- Land-use category should be changed to snow/ice for grid points with snow>0
         ...
         !-- Old formulation with constant thermal conductivity for snow
         !thdifsn = 0.265/rhocsn
         IF ( debug_print ) THEN
           print *,'thdifsn, 0.265/rhocsn,keff,rhosn,fact,SNHEI_crit', &
                    i,j,thdifsn, 0.265/rhocsn,keff,rhosn,fact,SNHEI_crit
         ENDIF
         ...
         !-- Update snow fractions for new snow 
       ...
       !-- Now decide if snow mosaic should be applied
       if(snowfrac < 0.90) snow_mosaic = 1. 

       KEEP_SNOW_ALBEDO = 0.
       if(newsn > 0. ) then 
         !-- fraction of new snow is needed only for albedo
         SNOWFRACnewsn=MIN(1.,SNHEI/SNHEI_CRIT_newsn)
         IF (snowfracnewsn > 0.99) THEN
           !-- new snow
           KEEP_SNOW_ALBEDO = 1.
           !-- set snow_mosaic=0 because fresh snowfall covers 
           !-- the whole grid cell. 
           snow_mosaic=0.  
         ENDIF
       endif
       ...
       !-- Estimate znt for snow from VEGPARM table (snow/ice landuse), except for
       ...
       !-- Update snow and ice albedo for the new snow fraction
       !-- ALB dependence on snow depth?
       !   ALB_SNOW across Canada's forested areas is very low - 0.27-0.35, this
       !   causes significant warm biases. Limiting ALB in these areas to be 
       !   higher than 0.4 helps with these biases.. 
       if( snow_mosaic == 1.) then
         ALBsn=alb_snow
         !if(newsn > 0..and. snowfallac > SNHEI_CRIT_newsn) then
         !-- 20apr20 - ~1 cm of fresh snow overwrites properties of "old" snow
           !print *,'ALB for fresh snow = 0.75'
           !ALBsn= 0.75 ! ECMWF value for fresh snow
         if (newsn > 0.) then
           if(snowfallac > SNHEI_CRIT_newsn.and. snhei > 5.*snhei_crit .and. alb < 0.6) then
           ...
           !print *,'ALB increase to 0.7',alb_snow,snhei,snhei_crit,alb,i,j
             ALBsn = 0.7
           endif
         endif
         Emiss= emissn
         ...
         !if(newsn > 0..and. snowfallac > SNHEI_CRIT_newsn) then
           !-- 20apr20 - ~1 cm of fresh snow overwrites properties of "old" snow
           !print *,'No mosaic ALB for fresh snow = 0.75',snowfallac,SNHEI_CRIT_newsn,i,j
           !ALBsn= 0.75 ! ECMWF value for fresh snow
         if (newsn > 0.) then
           if(snowfallac > SNHEI_CRIT_newsn.and. snhei > 5.*snhei_crit .and. alb < 0.6) then
           !-- Albedo correction with fresh snow and deep snow pack
       ...
       IF ( debug_print ) THEN
         ...
         print *,'Snow on soil snhei,snowfrac ',i,j,snhei,snowfrac
       ENDIF
       ...
       !------------------------------------------------------------
       !  LAND portion not covered with snow
...
!--- input variables
           ...
           infwater,rho,vegfrac,lai,tso_dop,zshalf_dop,myj,       &
!--- soil fixed fields 
...
!--- output variables for snow-free portion
       ...
       !--- using updated value of snow albedo - ALB
       gswnew=GSWin*(1.-alb)
       ...
       ENDIF
         ...
         !-- Snow on LAND 
         if(snow_mosaic==1.)then
           snfr=1. ! snow fraction in SNOWSOIL
         else
           snfr=snowfrac
         endif
...
!--- input variables
            ...
            QKMS,TKMS,PC,CST,infwater,                          &
            RHO,VEGFRAC,ALB,ZNT,lai,tso_dop,zshalf_dop,         &
            MYJ,                                                &
!--- soil fixed fields
            QWRTZ,rhocs,dqm,qmin,ref,wilt,psis,bclh,ksat,       &
            sat,cn,zsmain,zshalf,DTDZS,DTDZS2,tbq,              & 
!--- constants
            ...
            KQWRTZ,KICE,KWT,                                    &
!--- output variables
            cap,thdif,fso,fsn,d9sn,deltsn,d9,snprim,thdifsn,    &
            rhocsn,snth,ilnb,snwepr,rsm,tsob,                   &
            soilm1d,ts1d,smfrkeep,keepfr,                       &
            dew,soilt,soilt1,tsnav,qvg,qsg,qcg,                 &
            SMELT,SNEVAP,SNOH,SNFLX,SNOM,edir1,ec1,ett1,eeta,   &
            qfx,hfx,s,sublim,prcpl,fltot,runoff1,runoff2,       &
            mavail,soilice,soiliqw,infiltr)

       if (snow_mosaic==1.) then
       !-- May 2014 - now combine snow covered and snow-free land fluxes, 
       !   soil temp, moist, etc.
       IF ( debug_print ) THEN
       ...
       !-- Update snow density if there was melting.
       !-- If there was snow melting then snow pack has some liquid water,
       !-- and snow density has to be updated.
       ...
       if(smelt.gt.0..and.rsm.gt.0.) then
         ...
         !print *,'1- Melt rhosn',i,j,rhosn,snwe,rsm
           xsn=(rhosn*(snwe-rsm)+1.e3*rsm)/                         &
               snwe
           rhosn=MIN(MAX(58.8,xsn),500.)
         !print *,'2- Melt rhosn',i,j,rhosn,snwe,rsm
         endif
       endif

       if(SMELT > 0. .or. abs(SNEVAP) > 0.) then
       !-- Update SWE and snow depth for snowmelt and sublimation 
         SNWE = AMAX1(0.,(SNWE - (SMELT+SNEVAP)*DELT) )
         SNHEI= SNWE * rhowater / RHOSN
       endiF


       !-- Compute average temperature of the snow pack (C)
       IF(SNHEI.GT.0.) THEN
         if(ilnb.gt.1) then
           tsnav=0.5/snhei*((soilt+soilt1)*deltsn                     &
                     +(soilt1+ts1d(1))*(SNHEI-DELTSN))                 &
                     -273.15
         else
           tsnav=0.5*(soilt+ts1d(1)) - 273.15
         endif
       ELSE
         tsnav= soilt - 273.15
       ENDIF
     ...
     ELSE !--- no snow
         ...
         smelt=0.
         rhosn = 300. ! a fill value for snow-free areas
...
!--- input variables
            ...
            PRCPMS,RAINF,PATM,QVATM,QCATM,GLW,GSWnew,GSWin,     &
            EMISS,RNET,QKMS,TKMS,PC,cst,infwater,               &
            rho,vegfrac,lai,tso_dop,zshalf_dop,myj,             &
!--- soil fixed fields 
            QWRTZ,rhocs,dqm,qmin,ref,wilt,                      &
            psis,bclh,ksat,sat,cn,                              &
            zsmain,zshalf,DTDZS,DTDZS2,tbq,                     &
...
!--- input variables
            ...
            QKMS,TKMS,PC,cst,infwater,rho,vegfrac,lai,       &
            tso_dop,zshalf_dop,myj,                          &
!--- soil fixed fields
            QWRTZ,rhocs,dqm,qmin,ref,wilt,psis,bclh,ksat,    &
            sat,cn,zsmain,zshalf,DTDZS,DTDZS2,tbq,           &
!--- constants
...
!**************************************************************
! Compute soil resistance to evaporation.
!**************************************************************
!--- Sakaguchi and Zeng (2009) - dry soil resistance to evaporation
!--- 20jun18 - alfa is the ratio of surface Q to the saturation RH
!           qvg = alfa * qsg
        ...
        alfa=1.
!
!--- 20jun18 - beta in Eq. (4) is called soilres - it limits soil evaporation
! when soil moisture is below field capacity (ref).  [Lee and Pielke, 1992]
...
!---  For now we'll go back to ref*0.5
        ...
        !-- 20jun18 - soil moisture is higher than field capacity or condensation process.
          soilres = 1.
        else
...
!--- input variables
             ...
             QKMS,TKMS,PC,rho,vegfrac, lai,tso_dop,zshalf_dop,&
...
!-- input
               ...
               QKMS,TRANSP,DEW,0.,SOILICE,VEGFRAC,             &
               0.,soilres,zshalf_dop,                          &
!-- soil properties
        ...
        ! ---  evaporation
          ...
          !-- moisture flux for coupling with MYJ PBL
            ...
            IF ( debug_print ) THEN
              ...
              print *,'EC1=',EC1,'ETT1=',ETT1,'CMC2MS=',CMC2MS
            ENDIF
            !-- actual moisture flux from RUC LSM
...
!--- output variables
             ...
             qvg,qsg,qcg,SMELT,SNEVAP,SNOH,SNFLX,SNOM,         &
             edir1,ec1,ett1,eeta,qfx,hfx,s,sublim,             &
             prcpl,fltot,runoff1,runoff2,mavail,soilice,       &
             soiliqw,infiltrp)
...
!--- Local variables
   ...
   REAL    ::  infiltrp, transum                               , &
               snth, newsn, fso, fsn, d9sn, d9                 , &
               tabs, t3, upflux, xinet                         , &
               beta,snprim,epdt,pp,tsob
...
!-----------------------------------------------------------------
        ...
        ENDDO
         ...
         IF(SNHEI.GE.DELTSN+SNTH) THEN
           if(snhei-deltsn-snth.lt.snth) then
             deltsn=0.5*(snhei-snth)
             IF ( debug_print ) THEN
         ...
         !-- the following loop is added to define the amount of frozen
         !-- water in soil if there is any
         DO K=1,NZS
         ...
         !-- Update smfrkeep for new soilice
         do k=1,nzs
           if(soilice(k).gt.0.) then
             smfrkeep(k)=soilice(k)
           else
...
!******************************************************************** 
        ...
        SNWEPR=SNWE
        EPDT = EPOT * RAS * DELT
        IF(EPDT.gt.0. .and. SNWE.LE.EPDT) THEN 
           BETA=SNWE/max(1.e-8,EPDT)
           SNWE=0.
        ENDIF
...
!--- input variables
             ...
             GLW,GSW,EMISS,RNET,tso_dop,zshalf_dop,           &
             QKMS,TKMS,PC,rho,vegfrac,                        &
             thdif,cap,drycan,wetcan,cst,                     &
             tranf,transum,dew, rhocsn,                       &
!--- soil fixed fields
             dqm,qmin,psis,bclh,                              &
...
!--- output variables
             fso,fsn,d9sn,d9,snprim,thdifsn,                  &
             rsm,tsob,tso,soilt,soilt1,tsnav,qvg,qsg,qcg,     &
             smelt,snevap,snoh,snflx,s,ilnb,x)

!************************************************************************
       !-- RECALCULATION OF DEW or TRANSP USING NEW VALUE OF QSG 
       DEW=0.
       ETT1=0.
       PP=PATM*1.E3
       EPOT = -FQ*(QVATM-QSG)
...
!-- input
        ...
        zsmain,zshalf,diffu,hydro,                          &
        QSG,QVG,QCG,QCATM,QVATM,-INFWATER,                  &
        QKMS,TRANSP,0.,SMELT,soilice,vegfrac,               &
        snowfrac,1.,zshalf_dop,                             &
!-- soil properties
        DQM,QMIN,REF,KSAT,RAS,INFMAX,                       &
!-- output
        SOILMOIS,SOILIQW,MAVAIL,RUNOFF1,                    &
...
!     1         (1.+h1)
        did = (zshalf_dop-ZSHALF(nzs))
        h1=DTDZS(2*nzs-3)*THDIF(nzs-1)*(ZSHALF(nzs)-ZSHALF(nzs-1))/did
        cotso(1)=h1/(1.+h1)
        rhtso(1)=(tso(nzs)+h1*(tso(nzs-1)-tso_dop))/(1.+h1)
        !if(i==515.and.j==220)print *,'TSO(nzs),cotso(1),rhtso(1)',i,j,tso(nzs),cotso(1),rhtso(1)
        !cotso(1)=0.
        !rhtso(1)=TSO(NZS)
        DO 33 K=1,NZS2
          KN=NZS-K
...
!--- Local variables
   ...
   REAL    ::  t3,upflux,xinet,ras,                              &
               xlmelt, beta,epot,xsn,ddzsn,x1sn,d1sn,d2sn,d9sn,r22sn
...
!     1         (1.+h1)
    ...
    33  CONTINUE
     ...
     ENDIF
     IF(SNHEI.LT.SNTH.AND.SNHEI > 0.) then
...
!************************************************************************
!--- THE HEAT BALANCE EQUATION (Smirnova et al. 1996, EQ. 21,26)
        !-- 18apr08-- nmelt is the flag for melting, and 
        !--SNOH is heat of snow phase changes
        nmelt=0
        SNOH=0.
 ...
 212  continue
      ...
      !--- TDENOM for snow
        ...
        PP=PATM*1.E3
        AA1=AA1/PP
        !-- SNOH - heat of snow phase change, =0. at iter=1
        BB=BB-SNOH/TDENOM
  ...
  100   BB=BB-AA*TX2
        AA=(AA*H+CC)/PP
        !-- resolve energy budget in VILKA: TS1 and QS1 are the solution
        CALL VILKA(TN,AA,BB,PP,QS1,TS1,TBQ,KTAU,i,j,iland,isoil)
        Q1=TX2+H*QS1
    IF (debug_print ) THEN
  ...
  200   CONTINUE
     ...
     !-- Solution for skin temperature SOILT
     SOILT=TS1
     IF ( debug_print ) THEN
     ...
     IF ( debug_print ) THEN
       print *,'SOILT,SOILT1,tso,TSOB,QSG',i,j,SOILT,SOILT1,tso,TSOB,QSG, &
               'nmelt=',nmelt
     ENDIF

      !-- skip melting part of energy budget on the second iteration
      !-- or if all snow can evaporate
      if(nmelt.eq.1 .or. beta < 1.) go to 220

      !--- IF SOILT > 273.15 F then melting of snow can happen
      ! if all snow can evaporate, then there is nothing to melt
      ! Solution of energy budget to determine energy available for melting.
      IF(SOILT.GT.273.15.AND.SNHEI.GT.0.) THEN
        nmelt = 1
        soiltfrac=snowfrac*273.15+(1.-snowfrac)*SOILT
        QSG=min(QSG, QSN(soiltfrac,TBQ)/PP)
        qsg=snowfrac*qsg+(1.-snowfrac)*qvg
        qvg=qsg
        !-- compute energy budget components
        T3      = STBOLT*TN*TN*TN
        ...
        !--- X is storage in the surface layer
        ...
        !-- SNOH is available energy of snow phase change
        ...
        !-- SMELT is melting rate in M/S
        SMELT= SNOH /XLMELT*1.E-3
        IF ( debug_print ) THEN
          print *,'1- SMELT',i,j,smelt
        ENDIF
        !-- rr - snow available for melting
        rr=max(0.,SNWE/delt-BETA*EPOT*RAS)
        SMELT=amin1(SMELT,rr)
        IF ( debug_print ) THEN
          print *,'2- SMELT',i,j,smelt
        ENDIF
        !18apr08 - Egglston limit from the field experiments
    ...
    ENDIF
        ...
        !-- From Koren et al. (1999) 13% of melted snow stays in the snow pack
        rsmfrac=min(0.18,(max(0.08,snwepr/0.10*0.13)))
        if(snhei > 0.01) then
          rsm=rsmfrac*smelt*delt
        else
        !-- do not keep melted water in snow pack if its depth is less that 1 cm
          rsm=0.
       endif
   ...
   ENDIF ! SOILT > 273.15
       ...
       !18apr08 - if nmelt=1 then the snow melt occurred. 
       !-- SNOH is computed, now go into the 2nd iteration 
       !--for energy budget solution 
       if(nmelt.eq.1) goto 212  ! second interation
220  continue 
       ! end of the iteration

       !-- no melting when all snow can evaporate
       if(beta < 1) goto 221

       !--  If ground surface temperature
       !--  is above freezing then snow can melt from the bottom. 
       IF(TSO(1).GT.273.15 .and. snhei > 0.) THEN
         ...
         !-- check if the new soil temperature solution under the snow is 
         !-- consistent with the snow fraction
       ...
       END IF  ! end TSO(1) >273
 221   CONTINUE

       !-- total snow melt
       SMELT = SMELT + SMELTG
       !-- snow sublimation
       SNEVAP = BETA*EPOT*RAS

       IF ( debug_print ) THEN
         print *, 'D9SN,SOILT,TSOB : ', D9SN,SOILT,TSOB
       ENDIF

       !--- X is a final storage in the surface layer
       ...
       X=X &
       ! X plus "heat" from snow and rain
...
!     QCATM,QVATM - cloud and water vapor mixing ratio
!                   at the first atm. level (kg/kg)
!     INFWATER - infiltration rate (precipitation,drip) in m/s,
!                negative downward
!     QKMS - exchange coefficient for water vapor in the
!              surface layer (m/s)
...
!--- input variables
   LOGICAL,  INTENT(IN   )   ::  debug_print
   REAL,     INTENT(IN   )   ::  DELT, zshalf_dop
...
!******************************************************************************
 ...
 118      format(6(10Pf23.19))
        ...
        !         DID)/DENOM
        !
        !12 June 2014 - low boundary condition: 
        ! 1 - zero diffusion below the lowest level; 
        ! 2 - soil moisture at the low boundary can be lost due to the root uptake.
        ! So far - no interaction with the water table.

        !--DID=(ZShalf_dop-ZSHALF(NZS))
        !--X1=ZSMAIN(NZS)-ZSMAIN(NZS1)

        !--DENOM=(1.+DIFFU(nzs1)/X1/DID*DELT+HYDRO(NZS)/DID*DELT)
  ...
  191   format (f23.19)
          ...
          !-- TOTLIQ is negative downward
          TOTLIQ=INFWATER-UMVEG*DEW*RAS-SMELT
          IF ( debug_print ) THEN
            print *,'UMVEG*INFWATER,UMVEG*DEW*RAS,SMELT', &
                     UMVEG*INFWATER,UMVEG*DEW*RAS,SMELT
          ENDIF
...
!-- Constants for Johansen (1975) thermal conductivity
         ...
         DO K=1,NZS1
           ...
           !-- CAP - apparent volumetric heat capacity of soil
           CAP(K)=(1.-WS)*RHOCS                                    &
               + (soiliqwm(K)+qmin)*CVW                            &
               + soilicem(K)*CI                                    &
               + (ws-soilmoism(k))*CP*1.2                          &
               - DETAL(K)*rhowater*xlmelt

           !-- Johansen thermal conductivity KJPL
           kasat(k)=kas**(1.-ws)*kice**fwsat(k)                    &
                    *kwt**lwsat(k)

           X5=min(1.,(soilmoism(k)+qmin)/ws)
           if(soilicem(k) <= 0.) then
           !-- unfrozen soil
             sr=max(0.1,x5)
             ke=max(0.,log10(sr)+1.)
             !-- coarse soils
           ...
           !--- Diffusional soil moisture conductivity 
           a=RIW*soilicem(K)
           ...
           if((ws-a).lt.0.12)then
             ...
             facd=1.
             if(a > 0.)facd=1.-a/max(1.e-8,soilmoism(K))
         ...
         if((ws-riw*soilice(k)).lt.0.12)then
           ...
           fach=1.
           if(soilice(k) > 0.)                                     &
             fach=max(0.,(1.-riw*soilice(k)/max(1.e-8,soilmois(k))))
...
!beka        perturb hydrolic conductivity by 10-30%, not more than 50%
!         if (spp_lsm==1) then 
!         DO K=1,NZS !lala
...
!! Smirnova et al.(1997) \cite Smirnova_1997)
           SUBROUTINE TRANSF(  debug_print, i,j,                 &
!--- input variables
              nzs,nroot,soiliqw,tabs,lai,gswin,                  &
!--- soil fixed fields
              dqm,qmin,ref,wilt,zshalf,pc,iland,                 &
...
!***********************************************************************
     ...
     do k = 1,nlcat
       ...
       !-- 20aug18 - change in LAItoday based on the greenness fraction for the current day
         ...
         LAItoday(k) = LAITBL(K)
         !ZNTtoday(k) = Z0TBL(K)
         !-- do not overwrite z0 over water with the table value
         ZNTtoday(k) = ZNT
       endif
     enddo
...
END MODULE module_sf_ruclsm_land
